...
struct segtreminplus {
	...
	inline void fixcate(int i, int j) {
		...
		arr3[i][j]= j;
		if (i > 0) {
			arr[i][j] = min(arr[i - 1][j * 2]+ arr2[i - 1][j * 2], arr[i - 1][j * 2 + 1] + arr2[i - 1][j * 2+1]);
			if (arr[i][j] == arr[i - 1][j * 2] + arr2[i - 1][j * 2]) {
				arr3[i][j] = arr3[i - 1][j * 2];
			}
			else {
				arr3[i][j] = arr3[i - 1][j * 2 + 1];
			}
		}
		arr[i][j] += arr2[i][j];
		if (i > 0) {
			arr2[i - 1][2 * j] += arr2[i][j];
			arr2[i - 1][2 * j+1] += arr2[i][j];
		}
		arr2[i][j] = 0;
	}
	inline point val(int start, int end) {
		if (end >= n) {
			end = n - 1;
		}
		if (start < 0) {
		...
		fori(i, curlogn) {
			...
			if ((end & 1) == 0) {
				min2(mini, arr[i][end] + arr2[i][end]);
				if (mini == arr[i][end] + arr2[i][end]) {
					s = arr3[i][end];
				}
				end--;
			...
			if ((start & 1)) {
				min2(mini, arr[i][start] + arr2[i][start])
				if (mini == arr[i][start] + arr2[i][start]) {
					s = arr3[i][start];
				}
					start++;
			}
	...
	inline void add(int start,int end, lol num) {
		fori(i, curlogn) {
			fixcate(i, end);
			fixcate(i, end-1);
			fixcate(i, start);
...
vector<vecl> mult(vector<vecl>a, vector<vecl> b) {
	vector<vecl> c(a.size(), vecl(b[0].size()));
	forin(i, a) {
		forin(j, a[0]) {
			forin(k, b[0]) {
...
int main() {
	...
	wne(stk) {
		...
		if (cons[cur].size()) {
			dist[cur] = n+m;
			forin(i, cons[cur]) {
				min2(dist[cur],dist[cons[cur][i]]+1)
			}
		}
		forin(i, bcons[cur]) {
			int oth =bcons[cur][i];
			count[oth]++;
			if (count[oth] == cons[oth].size()) {
				stk.push_back(oth);
			}
		}
		if (dist[cur] == 0) {

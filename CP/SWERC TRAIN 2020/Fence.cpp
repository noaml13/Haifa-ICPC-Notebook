...
vector<point> ps;
int last = -1;
bool ok(int cur, ld ang){
    point v(1,0);
    v = rotate(v, ang);
    pair<ld, int> mind = {INF, -1};
    loop(i,0,n) if(i!=cur){
        pair<ld, int> d = {ps[i] ^ v, i};
        chkmin(mind, d);
    }
    mind.x -= ps[cur] ^ v;
    last = mind.y;
...
pair<int, ld> find_angle(int cur, ld ang){
    ld l = 0, r = pi;
    while((r-l)>eps){
        ld mid = (l+r)/2;
        if (ok(cur, ang-mid)) l = mid;
        else r = mid;
    }
    if (r == pi){
        return find_angle(cur, ang + pi);
    }
    return {l, last};
}
int32_t main(){
    ...
    ld ang = pi;
    while(1){
        auto res = find_angle(cur, ang);
        int nxt = res.x;
        used[nxt] = 1;
        ang = res.y;
        segs[cur].y = segs[nxt].x = ang;
        ans += (nxt);
        cur = nxt;
        if (cur == 0) break;
    }

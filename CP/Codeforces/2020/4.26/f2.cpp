...
int solve(){
    ...
    loop(i,1,n+1){
        //cout<<"I: "<<i<<endl;
        loop(s,0,m){
            loop(pos,0,n+1) dp[i][s][pos] = INF;
            for(int part = s; ; part = (part-1)&s){
                //if (part==s) continue;
                int take = s^part;
                int last = sum[take];
                //cout<<"HI "<<take<<" "<<ind[take].size()<<endl;
                for(int pos:ind[take]){
                    if (mini[i-1][part][pos].x<last) {
                        if (dp[i][s][pos+1]>last){
                            dp[i][s][pos+1] = last;
                            op[i][s][pos+1] = {part, mini[i-1][part][pos].y};
                        }
                    }
                }
                if (part==0) break;
